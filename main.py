...
def root():
    """Tiny status page at the root URL — a target for an uptime monitor (so the
    free host doesn't spin down) that's also human-readable. No JS/deps; the page
    self-refreshes every 30s. The JSON lives at /health."""
    h = health()
    last = h["last_tick_ts"]
    last_str = (datetime.fromtimestamp(int(last) / 1000, tz=timezone.utc)
                .strftime("%Y-%m-%d %H:%M UTC")) if last else "—"
    mode_color = "#c0392b" if h["mode"] == "LIVE" else "#27ae60"
    halt = "⛔ HALTED" if h["halted"] else "running"
    return f"""<!doctype html><html><head><meta charset="utf-8">
<title>BTC Executor</title><meta http-equiv="refresh" content="30">
<style>body{{font-family:system-ui,sans-serif;max-width:560px;margin:40px auto;
padding:0 16px;color:#222}}.k{{color:#777}}.v{{font-weight:600}}li{{margin:6px 0}}
.mode{{color:{mode_color};font-weight:700}}</style></head><body>
<h2>🤖 BTC Executor</h2>
<p>Mode: <span class="mode">{h['mode']}</span> &middot; {halt}</p><ul>
<li><span class="k">Open positions:</span> <span class="v">{h['open_positions']}</span></li>
<li><span class="k">Orders / fills / rejected:</span> <span class="v">{h['orders']} / {h['fills']} / {h['rejected']}</span></li>
<li><span class="k">Realized P&amp;L today:</span> <span class="v">${h['realized_pnl_today']:,.2f}</span></li>
<li><span class="k">Binance credentials:</span> <span class="v">{'yes' if h['credentials'] else 'no'}</span></li>
<li><span class="k">Feed &rarr; brain:</span> <span class="v">{'on' if h['feed'] else 'off'}</span> (ticks {h['ticks']}, forwarded {h['forwarded']})</li>
<li><span class="k">Last tick:</span> <span class="v">{last_str}</span></li>
</ul><p style="color:#999;font-size:13px">Auto-refreshes every 30s &middot;
JSON at <a href="/health">/health</a></p></body></html>"""
